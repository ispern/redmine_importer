...
class ImporterController < ApplicationController
  ...
  def result
    ...
    CSV.parse(iip.csv_data, {:headers => true, :encoding => iip.encoding,
                           :quote_char => iip.quote_char, :col_sep => iip.col_sep}) do |row|
      ...
      begin
        ...
        warning += "When adding issue #{@failed_count} below, the #{@unfound_class} #{@unfound_key} was not found<br>"
        next
      end
      ...
      if update_issue
        begin
          ...
          if ignore_non_exist
            ...
            @failed_count += 1
            @failed_issues[@failed_count] = row
            warning += "Could not update issue #{@failed_count} below, no match for the value #{row[unique_field]} were found<br>"
            next
          end
          ...
          @failed_count += 1
          @failed_issues[@failed_count] = row
          warning += "Could not update issue #{@failed_count} below, multiple matches for the value #{row[unique_field]} were found<br>"
          next
        end
      end
      ...
      issue.subject = row[attrs_map["subject"]] != nil ? row[attrs_map["subject"]].force_encoding("UTF-8") : issue.subject

      # optional attributes
      issue.description = row[attrs_map["description"]] != nil ? row[attrs_map["description"]].force_encoding("UTF-8") : issue.description
      ...
      begin
        ...
        if ignore_non_exist
          ...
          @failed_count += 1
          @failed_issues[@failed_count] = row
          warning += "When setting the parent for issue #{@failed_count} below, no matches for the value #{parent_value} were found<br>"
          next
        end
      rescue MultipleIssuesForUniqueValue
        @failed_count += 1
        @failed_issues[@failed_count] = row
        warning += "When setting the parent for issue #{@failed_count} below, multiple matches for the value #{parent_value} were found<br>"
        next
      end
      ...
      issue.custom_field_values = issue.available_custom_fields.inject({}) do |h, cf|
        if value = row[attrs_map[cf.name]]
          begin
            ...
            warning += "When trying to set custom field #{cf.name} on issue #{@failed_count} below, value #{value} was invalid<br>"
          end
        end
        h
      end
      ...
      if watchers
        ...
        watchers.split(',').each do |watcher|
          begin
            ...
            warning += "When trying to add watchers on issue #{@failed_count} below, User #{watcher} was not found<br>"
          end
        end
      end
      ...
      if (!issue.save)
        ...
        @failed_count += 1
        @failed_issues[@failed_count] = row
        warning += "The following data-validation errors occurred on issue #{@failed_count} in the list below<br>"
        issue.errors.each do |attr, error_message|
          warning += "&nbsp;&nbsp;"+error_message + "<br>"
        end
      else
        if unique_field
